...
(cust_id number(3) constraint customers_cust_id_pk primary key,   
cust_name varchar2(20) constraint customers_cust_name_nn not null
);

create table items
(item_id number(4) constraint items_item_id_pk primary key,
name varchar2(20) constraint items_name_nn unique not null,
unit_price number(6,2),
available_quantity number(6)
);

create table orders
(order_id number(4) constraint orders_order_id_pk primary key,
customer_id number(4) constraint orders_customer_id_fk references customers(cust_id),
order_date date);
...
item_id number(4),
quantity number,
constraint order_d_order_id_pk PRIMARY key(order_id,item_id)
);
